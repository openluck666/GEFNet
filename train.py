...
def run(args):
    ...
    # ---------------------------------main_model--------------------------------
    if args.MODEL.choice == "TFFMEF":
        model = TFFMEF.FFMEF(channels=args.MODEL.channels)
    model.cuda(local_rank)
    print(model,file=open('TFFMEF.txt','w'))
    #exit()
    # ----------------------------criterion-----optimizer-----LR_Schedule------------------------------
    #optimizer = torch.optim.AdamW(model.parameters(),lr=args.TRAIN.BASE_LR)
    # 渐进式学习率
    optimizer = torch.optim.Adam([
        {'params': model.parameters(), 'lr':args.TRAIN.BASE_LR }
    ])
    ...
    #flops, params = profile(model, (test_RGB,  test_RGB))
